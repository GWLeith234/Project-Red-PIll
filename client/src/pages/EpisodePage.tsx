  ...
  return (
    <div className="mb-8 rounded-xl overflow-hidden bg-black shadow-2xl" data-testid="video-player">
      ...
      <div className="bg-background px-4 py-3">
        <p className="text-foreground text-sm font-semibold truncate">{episode.title}</p>
        <p className="text-xs text-muted-foreground mt-1">Audio continues when you leave this page</p>
      </div>
    </div>
  );
}
...
function AudioPlayerUI({ episode, podcast }: { episode: any; podcast: any }) {
  ...
  return (
    <div className="bg-background rounded-lg p-5 mb-8" data-testid="audio-player">
      {isVideoOnly && (
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-foreground text-sm font-semibold truncate" data-testid="text-player-title">{episode.title}</p>
          <p className="text-muted-foreground text-xs">{podcast?.title || "Podcast"}</p>
...
function SuggestedEpisodes({ episodeId, currentPodcastId }: { episodeId: string; currentPodcastId: string }) {
  ...
  if (isLoading) {
    return (
      <div className="mt-10 pt-8 border-t border-border" data-testid="suggested-episodes-loading">
        <Skeleton className="h-6 w-48 mb-4 bg-muted" />
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="flex gap-3">
              <Skeleton className="h-16 w-16 rounded-lg bg-muted flex-shrink-0" />
  ...
  return (
    <div className="mt-10 pt-8 border-t border-border" data-testid="suggested-episodes">
        ...
        {suggestions.map((item: any) => {
          ...
          return (
            <Link
              ...
              className="block"
            >
              <div className="group flex gap-3 p-3 rounded-lg border border-border hover:border-border hover:bg-muted transition-all cursor-pointer" data-testid={`suggested-episode-${ep.id}`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-foreground group-hover:text-blue-700 transition-colors line-clamp-2 leading-snug" data-testid={`text-suggested-title-${ep.id}`}>
                    {ep.title}
                  </p>
                  <div className="flex items-center gap-2 mt-1.5">
                    {pod && !item.sameShow && (
                      <span className="text-xs text-muted-foreground truncate">{pod.title}</span>
                    )}
                    {ep.duration && (
...
export default function EpisodePage() {
    ...
    return (
      <div className="bg-card">
        <div className="max-w-5xl mx-auto px-4 py-12">
          ...
          <Skeleton className="h-20 w-full mb-6 bg-muted" />
  ...
  if (error || !data) {
    return (
      <div className="bg-card flex items-center justify-center py-20">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-foreground mb-2">Episode not found</h1>
          <p className="text-muted-foreground mb-4">This episode doesn't exist or hasn't been published yet.</p>
  ...
  return (
    <div className="bg-card" data-testid="episode-page">
      <nav className="border-b border-border bg-background/50">
        <div className="max-w-5xl mx-auto px-4">
          <div className="flex items-center space-x-1 text-sm text-muted-foreground py-3">
            <Link href={`/show/${params.podcastId}`} className="hover:text-foreground cursor-pointer">{podcast?.title || "Show"}</Link>
            <ChevronRight className="h-3 w-3" />
            <span className="text-foreground font-medium truncate max-w-[300px]">{episode.title}</span>
          </div>
        </div>
      </nav>
        ...
        <div className="flex gap-8">
          <div className="flex-1 min-w-0 max-w-[720px]">
            <div className="mb-6">
              ...
              <h1 className="text-3xl sm:text-4xl font-bold text-foreground leading-tight mb-3" data-testid="text-episode-title">
                {episode.title}
              </h1>
              <div className="flex items-center text-sm text-muted-foreground mb-5">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="font-medium text-foreground/80">{podcast?.host || "Host"}</span>
                </div>
                ...
                <div className="space-y-4">
                  {articles.map((article: any) => (
                    <Link
                      ...
                      className="block"
                    >
                      <div className="group border border-border hover:border-border rounded-lg p-4 transition-colors cursor-pointer" data-testid={`link-article-${article.id}`}>
                        <h3 className="font-semibold text-foreground group-hover:text-blue-700 transition-colors mb-1" data-testid={`text-article-title-${article.id}`}>
                          {article.title}
                        </h3>
                        {article.description && (
            ...
            {otherContent.length > 0 && (
              <div className="mt-8" data-testid="episode-content">
                <h2 className="text-lg font-bold text-foreground mb-4">From This Episode</h2>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {otherContent.map((piece: any) => {
                    const Icon = contentTypeIcons[piece.type] || FileText;
                    return (
                      <div key={piece.id} className="border border-border rounded-lg p-3" data-testid={`content-piece-${piece.id}`}>
                        <Icon className="h-4 w-4 text-muted-foreground mb-2" />
                        <p className="text-xs font-semibold text-foreground line-clamp-2">{piece.title}</p>
                        <p className="text-[10px] text-muted-foreground uppercase mt-1">{piece.type.replace("_", " ")}</p>
                      </div>
                    );
                  })}
                </div>
            ...
            <footer className="mt-8 pt-8 border-t border-border">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="font-semibold text-foreground text-sm">{podcast?.host || "Host"}</p>
                    <p className="text-muted-foreground text-xs">{podcast?.title}</p>
                  </div>
                </div>
                <Link href={`/show/${params.podcastId}`}>
                  <Button variant="outline" size="sm" className="text-muted-foreground border-border hover:bg-muted" data-testid="button-more-episodes">
                    More from this Show
                  </Button>
                </Link>
              </div>
          ...
          <aside className="hidden lg:block w-[300px] shrink-0 print:hidden" data-testid="episode-sidebar">
            <div className="sticky top-20 space-y-6">
              ...
              <div className="border border-border bg-background rounded-lg p-4">
                ...
                {podcast?.description && (
                  <p className="text-xs text-muted-foreground leading-relaxed mb-3 line-clamp-4">{podcast.description}</p>
                )}
                <p className="text-xs text-muted-foreground">
                  {podcast?.subscribers ? `${(podcast.subscribers / 1000).toFixed(0)}K subscribers` : ""}
                </p>
                <Link href={`/show/${params.podcastId}`}>
                  <Button variant="outline" size="sm" className="w-full mt-3 text-xs border-border" data-testid="button-sidebar-stories">
                    View All Content
                  </Button>
                </Link>
              </div>
              ...
              {episode.publishedAt && (
                <div className="border border-border bg-background rounded-lg p-4">
                  <h3 className="text-sm font-bold text-foreground uppercase tracking-wider mb-2">Episode Details</h3>
                  <dl className="space-y-2 text-xs">
                    <div>
                      <dt className="text-muted-foreground">Published</dt>
                      <dd className="text-foreground/80 font-medium">{formatDate(episode.publishedAt)}</dd>
                    </div>
                    {episode.duration && (
                      <div>
                        <dt className="text-muted-foreground">Duration</dt>
                        <dd className="text-foreground/80 font-medium">{formatDuration(episode.duration)}</dd>
                      </div>
                    )}
                    {contentPieces && contentPieces.length > 0 && (
                      <div>
                        <dt className="text-muted-foreground">Content Generated</dt>
                        <dd className="text-foreground/80 font-medium">{contentPieces.length} pieces</dd>
                      </div>
                    )}
                  </dl>
                </div>
