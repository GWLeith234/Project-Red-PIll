...
export default function AuthorProfile() {
      ...
      <div className="bg-card min-h-screen">
        <div className="max-w-5xl mx-auto px-4 py-12">
          <div className="flex items-start gap-6 mb-10">
            <Skeleton className="h-24 w-24 rounded-full bg-muted flex-shrink-0" />
            <div className="flex-1">
              <Skeleton className="h-8 w-48 mb-3 bg-muted" />
              <Skeleton className="h-4 w-32 mb-4 bg-muted" />
              <Skeleton className="h-4 w-full mb-2 bg-muted" />
              <Skeleton className="h-4 w-3/4 bg-muted" />
            </div>
          </div>
          <Skeleton className="h-6 w-40 mb-6 bg-muted" />
          {[1, 2, 3].map(i => (
  ...
  if (error || !data?.author) {
    return (
      <div className="bg-card flex items-center justify-center py-20 min-h-screen">
        <div className="text-center">
          <User className="h-16 w-16 text-foreground/80 mx-auto mb-4" />
          <h1 className="text-2xl font-bold text-foreground mb-2">Author not found</h1>
          <p className="text-muted-foreground">This author profile doesn't exist.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-card min-h-screen" data-testid="author-profile-page">
          ...
          <div className="flex items-start gap-6">
            {author.profilePhoto ? (
              <img
                ...
                className="h-24 w-24 rounded-full object-cover border-2 border-border flex-shrink-0"
                data-testid="img-author-photo"
              />
            ) : (
              <div className="h-24 w-24 rounded-full bg-background flex items-center justify-center flex-shrink-0" data-testid="img-author-placeholder">
                <User className="h-10 w-10 text-foreground" />
              </div>
            )}
            <div className="flex-1 min-w-0">
              <h1 className="text-3xl font-bold text-foreground mb-1" data-testid="text-author-name">
                {author.displayName || "Unknown Author"}
              </h1>
              {author.title && (
                <p className="text-lg text-muted-foreground mb-3" data-testid="text-author-title">{author.title}</p>
              )}
              {author.bio && (
                <p className="text-foreground/80 leading-relaxed max-w-2xl" data-testid="text-author-bio">{author.bio}</p>
              )}
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <h2 className="text-xl font-bold text-foreground mb-6 flex items-center gap-2" data-testid="text-articles-heading">
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link
                      ...
                      className="block"
                    >
                      <h3 className="text-lg font-semibold text-foreground group-hover:text-blue-600 transition-colors leading-tight mb-1.5 cursor-pointer" data-testid={`text-article-title-${article.id}`}>
                        {article.title}
                      </h3>
                    </Link>
                    {article.summary && (
                      <p className="text-sm text-muted-foreground leading-relaxed mb-2 line-clamp-2">{article.summary}</p>
                    )}
                    {article.description && !article.summary && (
                      <p className="text-sm text-muted-foreground leading-relaxed mb-2 line-clamp-2">{article.description}</p>
                    )}
                    <div className="flex items-center gap-3 text-xs text-muted-foreground">
                      {article.podcast && (
                        <span className="font-medium text-muted-foreground">{article.podcast.title}</span>
                      )}
                      {article.publishedAt && (
                        <>
                  ...
                  >
                    <ArrowRight className="h-5 w-5 text-foreground/80 group-hover:text-blue-500 transition-colors cursor-pointer" />
                  </Link>
                </div>
              </article>
            ))}
