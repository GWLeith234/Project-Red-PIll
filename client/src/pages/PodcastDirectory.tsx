...
export default function PodcastDirectory() {
  ...
  return (
    <div className="bg-background min-h-screen" data-testid="podcast-directory">
      <div className="bg-gradient-to-b from-background via-background to-background text-foreground relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 pt-12 pb-8 sm:pt-16 sm:pb-10 relative z-10">
          <div className="text-center max-w-2xl mx-auto mb-8">
            <div className="inline-flex items-center gap-2 bg-card/10 backdrop-blur-sm rounded-full px-4 py-1.5 mb-5">
              <Headphones className="h-4 w-4 text-red-400" />
              <span className="text-sm font-medium text-foreground/80">Podcast Network</span>
            </div>
            <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight mb-4 bg-gradient-to-r from-white via-white to-muted-foreground bg-clip-text text-transparent" data-testid="text-directory-title">
              Our Shows
            </h1>
          ...
          <div className="max-w-xl mx-auto relative mb-6">
            ...
            <input
              ...
              className="w-full bg-card/10 backdrop-blur-md border border-white/20 rounded-full pl-12 pr-4 py-3.5 text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-red-500/50 focus:border-red-500/40 focus:bg-card/15 transition-all"
            ...
            {CATEGORIES.map((cat) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${
                  activeCategory === cat
                    ? "bg-red-600 text-foreground shadow-lg shadow-red-600/30"
                    : "bg-card/10 text-foreground/80 hover:bg-card/20 hover:text-foreground"
                }`}
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        {isLoading ? (
          <div>
            <div className="mb-12">
              <Skeleton className="h-7 w-48 mb-6 bg-muted" />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="flex flex-col items-center">
                  <Skeleton className="h-32 w-32 rounded-full bg-muted mb-3" />
          ...
          <>
            {filtered.length >= 3 && !searchQuery && (
              <div className="mb-14">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-2xl font-bold text-foreground" data-testid="text-featured-heading">Featured Shows</h2>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  {filtered.slice(0, 3).map((podcast: any, idx: number) => (
                    <Link key={podcast.id} href={`/show/${podcast.id}`} className="block group" data-testid={`card-featured-${podcast.id}`}>
                      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-background via-background to-background text-foreground aspect-[4/3] shadow-xl group-hover:shadow-2xl transition-shadow">
                        {podcast.coverImage ? (
                          <img
                        ...
                        <div className="absolute top-4 left-4 flex items-center gap-2">
                          <div className="h-8 w-8 rounded-full bg-red-600 flex items-center justify-center text-foreground font-bold text-sm shadow-lg">
                            {idx + 1}
                          </div>
                          {idx === 0 && (
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-5">
                          <div className="flex items-end gap-3">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="text-muted-foreground text-sm truncate">with {podcast.host}</p>
                              {podcast.subscribers && (
                          ...
                          {podcast.description && (
                            <p className="text-foreground/80 text-sm line-clamp-2 leading-relaxed mt-3">{podcast.description}</p>
                          )}
                        </div>
                      </div>
                    </Link>
            ...
            {(trending.length > 0 || networkStats) && !searchQuery && (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-10">
                {networkStats && (
                  <div className="bg-card rounded-2xl border border-border shadow-sm p-6" data-testid="network-stats-widget">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="text-sm font-bold text-foreground uppercase tracking-wider">Network Stats</h3>
                    </div>
                    <div className="grid grid-cols-3 gap-3">
                      <div className="text-center">
                        <p className="text-2xl font-extrabold text-foreground">{networkStats.totalShows}</p>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium mt-0.5">Shows</p>
                      </div>
                      <div className="text-center">
                        <p className="text-2xl font-extrabold text-foreground">{formatSubscribers(networkStats.totalSubs)}</p>
                        <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium mt-0.5">Subscribers</p>
                      </div>
                      <div className="text-center">
                        ...
                        <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium mt-0.5">Avg Growth</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {trending.length > 0 && (
                  <div className="lg:col-span-2 bg-card rounded-2xl border border-border shadow-sm p-6" data-testid="trending-shows-widget">
                    <div className="flex items-center gap-2 mb-4">
                      ...
                      <h3 className="text-sm font-bold text-foreground uppercase tracking-wider">Trending Now</h3>
                    </div>
                    <div className="space-y-2.5">
                      {trending.map((p: any, idx: number) => (
                        <Link key={p.id} href={`/show/${p.id}`} className="flex items-center gap-3 group hover:bg-muted rounded-xl p-2 -mx-2 transition-colors" data-testid={`trending-show-${p.id}`}>
                          <span className="text-lg font-black text-muted-foreground w-6 text-right tabular-nums">{idx + 1}</span>
                          {p.coverImage ? (
                            <img src={p.coverImage} alt="" className="h-10 w-10 rounded-full object-cover ring-1 ring-border flex-shrink-0" />
                          ) : (
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-semibold text-foreground truncate group-hover:text-red-600 transition-colors">{p.title}</p>
                            <p className="text-xs text-muted-foreground truncate">{p.host} · {formatSubscribers(p.subscribers)} subs</p>
                          </div>
            ...
            <div>
              <div className="flex items-center gap-3 mb-8">
                ...
                <h2 className="text-2xl font-bold text-foreground" data-testid="text-all-shows-heading">
                  ...
                  <Link key={podcast.id} href={`/show/${podcast.id}`} className="block group" data-testid={`card-show-${podcast.id}`}>
                    <div className="flex flex-col items-center text-center">
                      <div className="relative mb-3">
                        <div className="h-32 w-32 sm:h-36 sm:w-36 rounded-full overflow-hidden ring-2 ring-border group-hover:ring-red-500 group-hover:ring-4 transition-all duration-300 shadow-md group-hover:shadow-xl bg-muted">
                          {podcast.coverImage ? (
                            <img
                              src={podcast.coverImage}
                              alt={podcast.title}
                        ...
                        <div className="absolute -top-1 -left-1 h-7 w-7 rounded-full bg-background text-foreground text-xs font-bold flex items-center justify-center shadow-md ring-2 ring-white">
                          {idx + 1}
                        </div>
                        <div className="absolute bottom-1 right-1 h-9 w-9 rounded-full bg-red-600 text-foreground flex items-center justify-center shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-300 scale-75 group-hover:scale-100">
                          <Play className="h-4 w-4 ml-0.5" />
                        </div>
                      </div>
                      <h3 className="font-bold text-foreground text-sm leading-snug truncate max-w-full group-hover:text-red-600 transition-colors" data-testid={`text-show-title-${podcast.id}`}>
                        {podcast.title}
                      </h3>
                      <p className="text-muted-foreground text-xs mt-0.5 truncate max-w-full">with {podcast.host}</p>
                      ...
                      {podcast.category && (
                        <span className="mt-1 inline-block px-2 py-0.5 bg-muted text-muted-foreground text-[10px] font-medium rounded-full">{podcast.category}</span>
                      )}
                    </div>
                  </Link>
                ))}
