...
Provide comprehensive social listening intelligence including trending topics, key posts, content strategy recommendations, audience insights, and competitor analysis.`;
  ...
  app.post("/api/push/campaigns", requireAuth, async (req, res) => {
    const { name, title, body, iconUrl, clickUrl, imageUrl, targetSegment, scheduledAt, status } = req.body;
    if (!name || !title || !body) return res.status(400).json({ message: "Name, title, and body are required" });
    if (title.length > 65) return res.status(400).json({ message: "Title must be 65 characters or less" });
    if (body.length > 120) return res.status(400).json({ message: "Body must be 120 characters or less" });
  ...
  app.patch("/api/push/campaigns/:id", requireAuth, async (req, res) => {
    const campaign = await storage.getPushCampaign(req.params.id);
    if (!campaign) return res.status(404).json({ message: "Campaign not found" });
    if (campaign.status !== "draft" && campaign.status !== "scheduled") {
      return res.status(400).json({ message: "Can only edit draft or scheduled campaigns" });
    }
    const { title, body } = req.body;
    if (title && title.length > 65) return res.status(400).json({ message: "Title must be 65 characters or less" });
    if (body && body.length > 120) return res.status(400).json({ message: "Body must be 120 characters or less" });
    const updates: any = { ...req.body, updatedAt: new Date() };
    if (updates.scheduledAt) updates.scheduledAt = new Date(updates.scheduledAt);
    if (updates.status === "scheduled" && !updates.scheduledAt && !campaign.scheduledAt) {
      return res.status(400).json({ message: "Scheduled campaigns require a scheduled date" });
    }
    const updated = await storage.updatePushCampaign(req.params.id, updates);
    res.json(updated);
  });

  app.delete("/api/push/campaigns/:id", requireAuth, async (req, res) => {
    const campaign = await storage.getPushCampaign(req.params.id);
    if (!campaign) return res.status(404).json({ message: "Campaign not found" });
    if (campaign.status !== "draft") return res.status(400).json({ message: "Can only delete draft campaigns" });
    await storage.deletePushCampaign(req.params.id);
    res.json({ success: true });
  });

  app.post("/api/push/campaigns/:id/send", requireAuth, async (req, res) => {
    const campaign = await storage.getPushCampaign(req.params.id);
    if (!campaign) return res.status(404).json({ message: "Campaign not found" });
    if (campaign.status !== "draft" && campaign.status !== "scheduled") {
      return res.status(400).json({ message: "Campaign must be in draft or scheduled status to send" });
    }
    ...
    try {
      ...
      if (campaign.targetSegment === "new") {
        const sevenDaysAgo = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
        subs = allSubs.filter(s => s.createdAt && new Date(s.createdAt) >= sevenDaysAgo);
      } else if (campaign.targetSegment === "at_risk") {
        const thirtyDaysAgo = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
        subs = allSubs.filter(s => !s.lastUsed || new Date(s.lastUsed) < thirtyDaysAgo);
      }

      await storage.updatePushCampaign(campaign.id, { status: "sending", recipientCount: subs.length } as any);

      if (!process.env.VAPID_PUBLIC_KEY || !process.env.VAPID_PRIVATE_KEY) {
        await storage.updatePushCampaign(campaign.id, { status: "sent", sentAt: new Date(), recipientCount: subs.length, deliveredCount: 0, failedCount: subs.length } as any);
        return res.json({ message: "VAPID keys not configured - campaign marked as sent", recipientCount: subs.length, delivered: 0 });
      }
          ...
          try {
            await webpush.sendNotification(
              ...
              JSON.stringify({
                ...
                body: campaign.body,
                icon: campaign.iconUrl || "/icon-192.png",
            ...
            delivered++;
            await storage.incrementPushCampaignDelivered(campaign.id);
            await storage.createPushCampaignLog({ campaignId: campaign.id, subscriptionId: sub.id, status: "delivered" });
          } catch (err: any) {
            failed++;
            await storage.incrementPushCampaignFailed(campaign.id);
            await storage.createPushCampaignLog({ campaignId: campaign.id, subscriptionId: sub.id, status: "failed", errorMessage: err.message || "Unknown error" });
            if (err.statusCode === 410 || err.statusCode === 404) {
              await storage.deletePushSubscription(sub.endpoint);
            }
          }
      ...
      await storage.updatePushCampaign(campaign.id, { status: "sent", sentAt: new Date() } as any);
      res.json({ message: "Campaign sent", recipientCount: subs.length, delivered, failed });
    } catch (err: any) {
      await storage.updatePushCampaign(campaign.id, { status: "failed" } as any);
      res.status(500).json({ message: err.message || "Failed to send campaign" });
    }
  });

  app.post("/api/push/campaigns/:id/cancel", requireAuth, async (req, res) => {
    const campaign = await storage.getPushCampaign(req.params.id);
    if (!campaign) return res.status(404).json({ message: "Campaign not found" });
    if (campaign.status !== "scheduled") return res.status(400).json({ message: "Can only cancel scheduled campaigns" });
  ...
  app.get("/api/push/stats", requireAuth, async (req, res) => {
    ...
    const sentThisMonth = sentCampaigns.filter(c => c.sentAt && new Date(c.sentAt) >= startOfMonth);
    ...
    const clicksThisMonth = sentThisMonth.reduce((sum, c) => sum + (c.clickedCount || 0), 0);

    const newSubs = subs.filter(s => s.createdAt && new Date(s.createdAt) >= sevenDaysAgo).length;

    res.json({
      totalSubscribers: subs.length,
      segmentCounts: { all: subs.length, new: newSubs, engaged: subs.length, at_risk: 0 },
  ...
  app.get("/api/push/click/:campaignId/:subscriptionId", async (req, res) => {
    try {
      const campaign = await storage.getPushCampaign(req.params.campaignId);
      if (campaign) {
        await storage.incrementPushCampaignClicked(campaign.id);
        await storage.createPushCampaignLog({ campaignId: campaign.id, subscriptionId: req.params.subscriptionId, status: "clicked" });
        res.redirect(campaign.clickUrl || "/home");
      } else {
        res.redirect("/home");
      }
    } catch {
  ...
  const SEED_CONFIGS: Array<Record<string, any>> = [
    ...
    { pageKey: "push-campaigns", title: "Push Campaigns", iconName: "Bell", route: "/push", permission: "content.view", navSection: "monetization", sortOrder: 24, primaryActionLabel: "+ New Campaign", aiActionLabel: "AI Suggest", description: "Create and send push notification campaigns" },
